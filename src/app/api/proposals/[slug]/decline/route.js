...
export async function POST(req, { params }) {
  try {
    ...
    if (proposal.statusId === 6) return NextResponse.json({ error: 'Already declined' }, { status: 400 })
    if (proposal.statusId === 5) return NextResponse.json({ error: 'Cannot decline an approved proposal' }, { status: 400 })

    await prisma.proposal.update({
      where: { slug },
      data: { statusId: 6, statusUpdated: new Date() },
    })
